...
def submit_button_set(key_prefix):
    ...
    with col1:
        submit_button = st.button('답변 전송', key=f"{key_prefix}_submit", use_container_width=True)
    with col2:
        quit_button = st.button('작업 끝내기', key=f"{key_prefix}_quit", use_container_width=True)
...
def retry_button_set(key_prefix):
    col1, col2 = st.columns(2)
    with col1:
        retry_button = st.button('다시 답변 전송', key=f"{key_prefix}_retry", use_container_width=True)
    with col2:
        continue_button = st.button('다음 질문받기', key=f"{key_prefix}_continue", use_container_width=True)
...
audio_value = st.audio_input("Talky의 질문에 한국어 또는 영어로 편하게 말해보세요!")
